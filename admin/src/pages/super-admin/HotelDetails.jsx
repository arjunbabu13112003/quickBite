    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.25rem', flexWrap: 'wrap' }}>
              ...
              {/* Open status */}
              <span style={{
                background: hotel.isOpen ? 'var(--bg-success-subtle)' : 'var(--bg-subtle)',
                color: hotel.isOpen ? 'var(--text-success)' : 'var(--text-muted)',
                padding: '0.25rem 0.6rem',
                borderRadius: 'var(--radius-full)',
                fontSize: '0.72rem',
                fontWeight: '800',
              ...
              {/* Accepts orders */}
              <span style={{
                background: hotel.acceptsOrders ? 'var(--bg-info-subtle)' : 'var(--bg-subtle)',
                color: hotel.acceptsOrders ? 'var(--text-info)' : 'var(--text-muted)',
                padding: '0.25rem 0.6rem',
                borderRadius: 'var(--radius-full)',
                fontSize: '0.72rem',
                fontWeight: '800',
      ...
      <div style={{
        ...
        <div style={{
          ...
          <Star size={24} style={{ color: ratingSummary.ratingCount > 0 ? 'var(--text-warning)' : 'var(--text-subtle)', fill: ratingSummary.ratingCount > 0 ? 'var(--text-warning)' : 'none' }} />
        </div>
                    ...
                    recentOrders.map((order) => (
                      <tr key={order.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                        ...
                        <td style={{ padding: '0.8rem 0.75rem' }}>
                          <span style={{
                            background: order.orderStatus === 'delivered' ? 'var(--bg-success-subtle)' : order.orderStatus === 'cancelled' || order.orderStatus === 'rejected' ? 'var(--bg-danger-subtle)' : 'var(--bg-info-subtle)',
                            color: order.orderStatus === 'delivered' ? 'var(--text-success)' : order.orderStatus === 'cancelled' || order.orderStatus === 'rejected' ? 'var(--text-danger)' : 'var(--text-info)',
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <div style={{ display: 'flex', color: 'var(--text-warning)', gap: '0.1rem' }}>
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} size={14} fill={i < Math.round(ratingSummary.averageRating) ? 'var(--text-warning)' : 'none'} strokeWidth={1.5} />
                      ))}
                    </div>
                    ...
                    return (
                      <div key={star} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', fontSize: '0.78rem' }}>
                        ...
                        <div style={{ flex: 1, height: '6px', background: 'var(--bg-main)', borderRadius: '3px', overflow: 'hidden' }}>
                          <div style={{ width: `${pct}%`, height: '100%', background: 'var(--text-warning)', borderRadius: '3px' }}></div>
                        </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
                {admins.map((admin) => (
                  <div key={admin.id} style={{
                    ...
                    <span style={{
                      background: admin.isActive ? 'var(--bg-success-subtle)' : 'var(--bg-subtle)',
                      color: admin.isActive ? 'var(--text-success)' : 'var(--text-muted)',
