...
export default function HotelsList({ onNavigate }) {
  ...
  const renderStoreStatusBadge = (isOpen) => {
    return (
      <span style={{
        background: isOpen ? 'var(--bg-success-subtle)' : 'var(--bg-subtle)',
        color: isOpen ? 'var(--text-success)' : 'var(--text-muted)',
        padding: '0.25rem 0.6rem',
        borderRadius: 'var(--radius-full)',
        fontSize: '0.72rem',
        fontWeight: '850',
              ...
              {currentRows.length > 0 ? (
                currentRows.map((hotel) => (
                  <tr key={hotel.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                    ...
                    {/* Accepting Orders */}
                    <td style={{ padding: '0.9rem 1rem', fontSize: '0.88rem', fontWeight: '750', color: hotel.acceptsOrders ? 'var(--text-success)' : 'var(--text-muted)' }}>
